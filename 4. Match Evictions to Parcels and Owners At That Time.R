...
for (i in 1:length(durham_apts_sliced_owners$End_Date)) {
  if (is.na(durham_apts_sliced_owners$End_Date[i]) == TRUE) {
    durham_apts_sliced_owners$End_Date[i] = as.Date("2018-31-01")
  }
}
...
durham_apts_sliced_owners <- durham_apts_sliced_owners[row.names(unique(durham_apts_sliced_owners[,c("PARCEL_ID", "Umbrella.Name",
                                                                        "Start_Date","End_Date")])),]
...
#NOTE RESTRICTION FOR NOW:  
merged_data = merged_data %>% filter(year >= 2004)
...
apt_evictions_eda <- apt_evictions_eda[!duplicated(apt_evictions_eda[,c("PARCEL_ID","Umbrella.Name",
                                                   "Start_Date","End_Date")]),]
